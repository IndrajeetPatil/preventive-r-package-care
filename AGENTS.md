...
- **Section rhythm.** Every content section follows the same shape: `# Section {#anchor}` with a one-line "Preventive care …" tagline, an attributed quote slide, a `# <span class="visually-hidden">…</span> {#section-N}` overview slide wrapping the green box, then `# Make sure ….` divider slides that mirror that section's checklist items one-to-one. Problem slides build up with `. . .` fragments and end on a red `**How to …?**` question before the slide that answers it. Each section also has a `## Checklist for … {.smaller}` slide in the early checklist run.
