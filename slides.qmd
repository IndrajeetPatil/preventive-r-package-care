...
**To reduce maintenance workload, make sure that checks pass both in the presence and absence of soft dependencies.**
...
Preventive care to make sure your package works across diverse contexts.
...
You (the developer) may be working on the package in a certain setting; i.e. with version of R and OS, in a certain locale, etc. Even if all tests pass and all examples run successfully for you locally, you can't assume that your users will use the package in similar settings.
...
If you restrict your checks only to run on these specific settings, you may not anticipate the problems your users may experience in other settings (e.g., the graphics device that works on `windows` may not work on `macOS`, code that works with `R 4.1` may not work with `R 3.6`, etc.). Of course, the assumption here is that your package claims to support these settings.
