...
You can use [`{covr}`](https://covr.r-lib.org/) to compute code coverage via examples in help pages and vignettes, and to ensure that it is above a certain threshold.
...
You can write a GHA workflow to automate this check (see [example workflow](https://github.com/IndrajeetPatil/statsExpressions/blob/main/.github/workflows/test-coverage-examples.yaml){target='_blank'}). This will ensure that the code coverage via examples never drops below the chosen threshold.
...
- If you want to programmatically convert your existing `.md` to `.Rmd`, you are [currently](https://github.com/yihui/knitr/issues/1807) out of luck. You will need to do this manually.
