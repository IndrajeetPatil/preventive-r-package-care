...
It is important to anticipate changes in hard dependencies that may break your packages. Although most developers will be kind enough to let you know of upcoming breaking changes, such communication can't be assumed. Additionally, if your package is not on CRAN, there is no way for the maintainer to detect possible breakage and inform you. Ditto if you are skipping tests on CRAN.
...
This means that the only way you may figure out that something is broken is *after* the package is released, and most likely via frustrated users.
