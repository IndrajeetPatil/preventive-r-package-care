...
In larger projects or teams, where multiple contributors may be working on the same codebase, it can become difficult for an individual contributor to detect code quality or formatting issues that they or someone else may have introduced.
...
- Most lints related to code formatting can be removed using [`{styler}`](https://r-lib.github.io/styler/){target='_blank'}, which enforces [tidyverse style guide](https://style.tidyverse.org/){target='_blank'}. Preferably code is formatted consistently on each commit using [GHA workflow](https://github.com/r-lib/actions/blob/v2-branch/examples/style.yaml){target='_blank'}.

- Code smells are subjective and you may disagree with some linters. Use a [configuration file](https://lintr.r-lib.org/dev/articles/lintr.html) to customize which linters you wish to include. Additionally, you can annotate parts of code that linters should ignore.
...
| Item                                                  |
| :---------------------------------------------------- |
| Make sure code follows style guide.                   |
| Make sure there are no known code quality issues.     |
| Make sure there are no severe performance regressions |
