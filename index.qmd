...
- [Coding-agent readiness](#llm-readiness)
...
## Checklist for coding-agent readiness {.smaller}
...
# <span class="visually-hidden">Coding-agent readiness</span> {#section-6 aria-label="Coding-agent readiness: overview"}
<!-- column layout -->
...
## Where instructions live {.smaller}
...
Exclude all three from package tarballs via [`.Rbuildignore`](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Building-package-tarballs).
...
Put an [`AGENTS.md`](https://developers.openai.com/codex/guides/agents-md/) file at the repository root. It accompanies **every task**, so keep it short.
...
**For [an R package](https://github.com/IndrajeetPatil/statsExpressions/blob/50b387691bff7a04188fd9a8ec7beb4c4e68fda6/AGENTS.md), this might include:**
...
Link to a detailed procedure instead of copying it here: a link is read only when relevant, a copy is loaded every session.
...
A [skill](https://agentskills.io/what-are-skills) holds a task's detailed procedure in its own directory. Its `SKILL.md` contains a name, a description of when to use it, and the instructions.
...
**A skill for [preparing a CRAN release](https://github.com/IndrajeetPatil/statsExpressions/blob/50b387691bff7a04188fd9a8ec7beb4c4e68fda6/.agents/skills/create-release/SKILL.md):**
...
Keep [recurring requests](https://github.com/IndrajeetPatil/statsExpressions/tree/50b387691bff7a04188fd9a8ec7beb4c4e68fda6/.github/prompts) in plain Markdown files in the repository:
...
[Tag or attach](https://learn.chatgpt.com/docs/developer-commands#interactive-shortcuts) `@.github/prompts/update-deps.md` and send it. The file already contains the request.

:::

## Claude Code compatibility {.smaller}

Claude Code uses different locations for repository instructions and skills:

| Resource | Other coding agents | Claude Code |
| :--- | :--- | :--- |
| Repository instructions | `AGENTS.md` | [`CLAUDE.md`](https://code.claude.com/docs/en/memory#agentsmd) |
| Skill directories | `.agents/skills/` | [`.claude/skills/`](https://code.claude.com/docs/en/skills#where-skills-live) |
...
Symlink the Claude Code paths to the shared files and folders, so there is only one copy to maintain.

:::

# Make sure your documentation is machine-readable.

## Machine-readable docs: `llms.txt` {.smaller}
...
[pkgdown publishes this for you](https://pkgdown.r-lib.org/reference/build_llm_docs.html): an [`llms.txt`](https://llmstxt.org/) index at the site root, plus a Markdown twin of every rendered page.
...
Submit your repository at [context7.com/add-library](https://context7.com/add-library) so agents querying Context7, through its MCP server or CLI, look up your current documentation instead of recalling an older release from training data.
