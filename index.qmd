...
Start the coding agent from the repository root and ask it to read the file. Describe the requested change, its scope, and the expected result in the prompt.
...
For example, tell the agent: "Read `.github/prompts/update-deps.md` and follow its instructions." This directory is a repository convention; the request is read when you ask for it.
...
- Start a new agent session from the package's root directory and ask which repository instructions it loaded.

- Check that skills are discoverable and that the agent can read the prompt files you point it to.
