...
::: {style="text-align: center; font-size: 0.55em; margin-top: 1em;"}
Sources: [Codex repository instructions](https://developers.openai.com/codex/guides/agents-md/), [Agent Skills](https://developers.openai.com/codex/skills/), and [VS Code prompt files](https://code.visualstudio.com/docs/agent-customization/prompt-files).
:::
...
::: {style="text-align: center; font-size: 0.55em; margin-top: 1em;"}
Sources: [repository instructions](https://developers.openai.com/codex/guides/agents-md/), [Agent Skills](https://developers.openai.com/codex/skills/), and [prompt files](https://code.visualstudio.com/docs/agent-customization/prompt-files).
:::
...
::: {style="text-align: center; font-size: 0.55em; margin-top: 1em;"}
Source: [Codex instruction discovery and authoring guidance](https://developers.openai.com/codex/guides/agents-md/).
:::
...
**A skill for reviewing changes:**

```markdown
---
name: review-change
description: Review an R package change.
  Use when asked for a code review.
---

1. Read AGENTS.md and the diff.
2. Trace changes to public behavior.
3. Check relevant tests and documentation.
4. Report findings with file references.
5. Do not edit files during the review.
```
...
::: {style="text-align: center; font-size: 0.55em; margin-top: 1em;"}
Source: [skill structure, descriptions, and discovery](https://developers.openai.com/codex/skills/).
:::
...
Keep recurring requests in Markdown files. For VS Code's reusable prompt menu, use the `.prompt.md` extension:

```text
.github/prompts/
└── update-deps.prompt.md
```

You choose when to run the prompt. Describe the requested change, its scope, and the expected result.
...
For other agents, you can keep a plain `.md` request and explicitly ask the agent to read it. Check which file names and invocation methods your tool supports.

:::

::: {style="text-align: center; font-size: 0.55em; margin-top: 1em;"}
Source: [VS Code prompt files and invocation](https://code.visualstudio.com/docs/agent-customization/prompt-files).
:::
...
**For the review skill above, run from the repository root:**

```bash
ln -s AGENTS.md CLAUDE.md
mkdir -p .claude/skills
ln -s ../../.agents/skills/review-change \
  .claude/skills/review-change
```

::: {style="font-size: 20px;"}

Commit the links and repeat the skill link for each shared skill. Check existing destinations first.
...
::: {style="text-align: center; font-size: 0.55em; margin-top: 1em;"}
Sources: [Claude Code instruction files](https://code.claude.com/docs/en/memory#agentsmd) and [skill discovery and symlinks](https://code.claude.com/docs/en/skills#where-skills-live).
:::
...
- Check that skills are discoverable and that prompt files can be invoked or read explicitly.

- Try a small task to check whether the instructions give the agent enough context.

- Keep file references, task boundaries, and instructions current as the package changes.

. . .

::: {style="background-color: #e8f5e9; padding: 20px; border-radius: 25px;"}

Keep shared conventions in `AGENTS.md` and refer to them from skills and prompts. This reduces the number of places you need to update when a convention changes.

:::

::: {style="text-align: center; font-size: 0.55em; margin-top: 1em;"}
Sources: [instruction verification](https://developers.openai.com/codex/guides/agents-md/), [skill discovery](https://developers.openai.com/codex/skills/), and [prompt invocation](https://code.visualstudio.com/docs/agent-customization/prompt-files).
:::

## An example: `statsExpressions` {.smaller}

:::: {.columns}

::: {.column width='50%'}

[`statsExpressions`](https://github.com/IndrajeetPatil/statsExpressions) keeps shared instructions, a release procedure, and maintenance requests in the repository:
...
- **Instructions** explain how to preserve the returned columns and plotmath expressions that `ggstatsplot` consumes.

- **The release skill** describes the steps for a CRAN release, including waiting for acceptance before merging and publishing.

- **The prompts** describe dependency updates, code simplification, and review follow-up.
...
The prompts here are plain `.md` files to request explicitly. The inspected repository has no Claude Code compatibility links; add those if your team uses Claude Code.

:::

::: {style="text-align: center; font-size: 0.55em; margin-top: 1em;"}
Sources: [`statsExpressions` instructions](https://github.com/IndrajeetPatil/statsExpressions/blob/50b387691bff7a04188fd9a8ec7beb4c4e68fda6/AGENTS.md), [release skill](https://github.com/IndrajeetPatil/statsExpressions/blob/50b387691bff7a04188fd9a8ec7beb4c4e68fda6/.agents/skills/create-release/SKILL.md), and [repository files](https://github.com/IndrajeetPatil/statsExpressions/tree/50b387691bff7a04188fd9a8ec7beb4c4e68fda6).
:::
